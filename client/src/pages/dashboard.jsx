...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on("quizSaved", (quizData) => {
      ...
      if (quizData.success) {
        // This could be a new quiz data push to a state array
        // or simply refetch queries to update the list with the newly added quiz
        refetch(); // Re-fetch quizzes to include the new one
      }
    });
    // Clean up the effect to prevent memory leaks
    return () => socket.off("quizSaved");
  }, [refetch]); // dependency on refetch function
